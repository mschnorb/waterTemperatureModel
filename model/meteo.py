...
class Meteo(object):
    ...
    def __init__(self, iniItems, landmask, spinUp):
        ...
        if self.refETPotMethod == 'Hargreaves':
            self.latitudes = pcr.ycoordinate(self.cloneMap)  # needed to calculate 'referenceETPot'
            self.tmpMaxFileNC = iniItems.meteoOptions['tmpMaxFileNC']
            self.tmpMinFileNC = iniItems.meteoOptions['tmpMinFileNC']
        ...
        if self.report == True:
            ...
            if self.outDailyTotNC[0] != "None":
                for var in self.outDailyTotNC:
                    ...
                    self.netcdfObj.createNetCDF(str(self.outNCDir)+ "/" + str(var) + "_dailyTot.nc", var, "undefined")
            ...
            if self.outMonthTotNC[0] != "None":
                for var in self.outMonthTotNC:
                    ...
                    # creating the netCDF files:
                    self.netcdfObj.createNetCDF(str(self.outNCDir)+ "/" + str(var) + "_monthTot.nc", var, "undefined")
            # - average
            if self.outMonthAvgNC[0] != "None":
                for var in self.outMonthAvgNC:
                    ...
                    # creating the netCDF files:
                    self.netcdfObj.createNetCDF(str(self.outNCDir) + "/" + str(var) + "_monthAvg.nc", var, "undefined")
            # - last day of the month
            if self.outMonthEndNC[0] != "None":
                for var in self.outMonthEndNC:
                    # creating the netCDF files:
                    self.netcdfObj.createNetCDF(str(self.outNCDir) + "/" + str(var) + "_monthEnd.nc", var, "undefined")
            ...
            if self.outAnnuaTotNC[0] != "None":
                for var in self.outAnnuaTotNC:
                    ...
                    # creating the netCDF files:
                    self.netcdfObj.createNetCDF(str(self.outNCDir) + "/" + str(var) + "_annuaTot.nc", var, "undefined")
            # - average
            if self.outAnnuaAvgNC[0] != "None":
                for var in self.outAnnuaAvgNC:
                    ...
                    vars(self)[var + 'AnnuaTot'] = None
                    # creating the netCDF files:
                    self.netcdfObj.createNetCDF(str(self.outNCDir) + "/" + str(var) + "_annuaAvg.nc", var, "undefined")
            # - last day of the year
            if self.outAnnuaEndNC[0] != "None":
                for var in self.outAnnuaEndNC:
                    # creating the netCDF files:
                    self.netcdfObj.createNetCDF(str(self.outNCDir) + "/" + str(var) + "_annuaEnd.nc", var, "undefined")
    ...
    def read_forcings(self, currTimeStep):
        ...
        # reading temperature
        self.temperature = vos.netcdf2PCRobjClone(self.tmpFileNC,
                                                  self.temperatureVarName,
                                                  str(currTimeStep.fulldate),
                                                  useDoy=None,
                                                  cloneMapFileName=self.cloneMap,
                                                  LatitudeLongitude=True)
        ...
        if self.refETPotMethod == 'Hargreaves':
            self.temperatureMax = vos.netcdf2PCRobjClone(self.tmpMaxFileNC,
                                                         'tmax',
                                                         str(currTimeStep.fulldate),
                                                         useDoy=None,
                                                         cloneMapFileName=self.cloneMap,
                                                         LatitudeLongitude=True)
            self.temperatureMin = vos.netcdf2PCRobjClone(self.tmpMinFileNC,
                                                         'tmin',
                                                         str(currTimeStep.fulldate),
                                                         useDoy=None,
                                                         cloneMapFileName=self.cloneMap,
        ...
        if self.refETPotMethod == 'Hamon':
            self.referencePotET = refPotET.HamonPotET(self.temperature,
                                                      currTimeStep.doy,
                                                      self.latitudes)

        if self.refETPotMethod == 'Hargreaves':
            self.referencePotET = refPotET.HargreavesPotET(self.temperatureMax,
                                                           self.temperatureMin,
                                                           currTimeStep.doy,
                                                           self.latitudes)

        if self.refETPotMethod == 'Input':
            self.referencePotET = vos.netcdf2PCRobjClone(self.etpFileNC,
                                                         self.evaporationVarName,
                                                         str(currTimeStep.fulldate),
                                                         useDoy=None,
                                                         cloneMapFileName=self.cloneMap,
                                                         LatitudeLongitude=True)
