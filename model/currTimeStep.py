...
class ModelTime(object):
    ...
    def isLastDayOfSubSeason(self):
        tomorrow = self.currTime + datetime.timedelta(days=1)
        
        #tomorrow is the first day of the month
        return tomorrow.day == 1 or tomorrow.day == 16

    def subSeasonLength(self):
        tomorrow = self.currTime + datetime.timedelta(days=1)
        if self.isLastDayOfSubSeason() and tomorrow.day == 1:
            totalDays = self.currTime.day - 15
        else:
            totalDays = 15
        #tomorrow is the first day of the month
        return totalDays
    ...
    @property
    def seasonLength(self):
        return self.subSeasonLength()

    #FIXME: use isLastDayOfMonth
    @property
    def endSubSeason(self):
        return self.isLastDayOfSubSeason()
