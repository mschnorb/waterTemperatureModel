...
class RoutingModel(object):
    ...
    def __init__(self, configuration, currTimeStep, initialState=None):
        ...
        # Read the ldd map.
        self.lddMap = vos.readPCRmapClone(configuration.routingOptions['lddMap'],
    ...
    def dumpState(self, outputDirectory):
        ...
        for variable, map in routingState.items():
            vos.writePCRmapToDir(map, str(variable)+"_"+str(self._modelTime.fulldate)+".map", outputDirectory)
    ...
    def report(self):
        ...
        if self._modelTime.isLastDayOfMonth():
            ...
            variableList = ['precipitation', 'baseflow', 'runoff']

            for var in variableList:
                volume = vos.getMapVolume(self.__getattribute__(var + 'Acc'), self.routing.cellArea)
                msg = 'Accumulated %s days 1 to %i in %i = %e km3 = %e mm' % \
                      (var, int(self._modelTime.doy), int(self._modelTime.year), volume/1e9, volume*1000/totalCellArea)
                logging.getLogger("model").info(msg)
    ...
    def checkWaterBalance(self, storesAtBeginning, storesAtEnd):
        ...
        precipitation = pcr.ifthen(self.landmask, self.meteo.precipitation)  # unit: m
