...
def netcdf2PCRobjClone(ncFile, varName, dateInput,
    ...
    if useDoy == "Yes":
        ...
        if useDoy == "month":
            ...
            try:
                ...
                try:
                    idx = nc.date2index(date, nctime, calendar=nctime.calendar, select='before')
                    msg = "\n"
                    ...
                    try:
                        idx = nc.date2index(date, nctime, calendar=nctime.calendar, select='after')
                        msg = "\n"
                        msg += "WARNING related to the netcdf file: "+str(ncFile)+" ; variable: "+str(varName)+" !!!!!!"+"\n"
    ...
    if not sameClone:
        ...
        logger.info('Resample: input cell size = ' + str(float(cellsizeInput)) + ' ; output/clone cell size = ' +
                    str(float(cellsizeClone)))
        factor = int(np.round(float(cellsizeInput)/float(cellsizeClone)))
...
def netcdf2PCRobjCloneMultiDim(ncFile, varName, dateInput,
    ...
    if not sameClone:
        ...
        # yIdxSta = int(np.where(f.variables['lat'][:] == yULClone - 0.5*cellsizeInput)[0])
        minY = min(abs(f.variables['lat'][:] - (yULClone - 0.5*cellsizeInput))) # ; print(minY)
        yIdxSta = int(0) #int(np.where(abs(f.variables['lat'][:] - (yULClone - 0.5*cellsizeInput)) == minY)[0])
        yIdxEnd = int(math.ceil(yIdxSta + rowsClone /(cellsizeInput/cellsizeClone)))
        cropData = f.variables[varName][idx,yIdxSta:yIdxEnd,xIdxSta:xIdxEnd,:]
    ...
    outPCR = np.zeros((cropData.shape[0]*factor, cropData.shape[1]*factor, cropData.shape[2]))

    # convert to PCR object and close f
    if specificFillValue is not None:
        outPCR = regridData2FinerGrid(factor, temp, MV)
        #for i in range(cropData.shape[0]):
            #outPCR[i,:,:] = regridData2FinerGrid(factor,cropData[i,:,:],MV)
    else:
        #outPCR = regridData2FinerGrid(factor,temp,MV)
        #print outPCR.shape
        for i in range(cropData.shape[2]):
            outPCR[:,:,i] = regridData2FinerGrid(factor,cropData[:,:,i],MV)
    #f.close();
...
def netcdf2PCRobjCloneWindDist(ncFile, varName, dateInput, useDoy=None, cloneMapFileName=None):
...
def netcdf2PCRobj(ncFile, varName, dateInput):
    ...
    # convert to PCR object and close f
    outPCR = pcr.numpy2pcr(pcr.Scalar,(f.variables[varName][idx].data), float(f.variables[varName]._FillValue))
...
def writePCRmapToDir(v, outFileName, outDir):
    ...
    # cloneMapFileName: If the inputMap and cloneMap have different clones, resampling will be done. Then,
...
def readPCRmapClone(v, cloneMapFileName, tmpDir,
                    ...
                    isNomMap=False):
    # v: inputMapFileName or floating values
    # cloneMapFileName: If the inputMap and cloneMap have different clones, resampling will be done.
...
def getMapAttributesALL(cloneMap, arcDegree=True):
    ...
    cOut, err = subprocess.Popen(co, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, shell=True).communicate()
    if err != None or cOut == []:
        print("Something wrong with mapattr in virtualOS, maybe clone Map does not exist ? ")
        sys.exit()
...
def waterBalanceCheck(fluxesIn, fluxesOut, preStorages, endStorages, processName, PrintOnlyErrors,
                      dateStr, threshold=1e-5, landmask=None):
